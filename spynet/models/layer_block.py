...
from theano.tensor.nnet import conv, conv3d2d, conv2d
...
class LayerBlockSum(LayerBlock):
    """
    Summation
    """
    name = "Summation Layer block"
    ...
    def forward(self, x, batch_size, run_time):
        # if not x.shape == self.in_block.output.shape:
        #     print("x shape: " + str(x.shape[0]))
        #     print("in_block output shape: " + str(self.in_block.output.shape[0]))
        #     print("shapes of the two inputs are not the same")
        # print(x)
        # i1 = T.dmatrix('i1')
        # i2 = T.dmatrix('i2')
        # output = i1 + i2
        # sum = function([i1, i2], output)


        output = x + self.in_block.output.reshape((batch_size,)+self.in_block.output_shape)
        self.in_block.output = None
        return output

class LayerBlockSumProjA(LayerBlock):
    """
    Summation with Projection A
    """
    name = "Summation Layer block with Projection A"
    ...
    def forward(self, x, batch_size, run_time):
        proj_output = self.in_block.output.reshape((batch_size, self.in_block.output_shape[0]*2, self.in_block.output_shape[1], self.in_block.output_shape[2]))[:, ::2, ::2]
        self.in_block.output = None
        proj_output = T.concatenate((proj_output, T.zeros((batch_size, self.in_block.output_shape[0]*2, self.in_block.output_shape[1]//2, self.in_block.output_shape[2]//2))), axis=0)
...
class LayerBlockConv2DAbstract(LayerBlockOfNeurons):
    ...
    """
    def __init__(self, neuron_type, in_shape, flt_shape, save_output=False, in_block=None):
        """
        Args:
...
class LayerBlockPadConv2DAbstract(LayerBlockOfNeurons):
    """
    Abstract class defining common components of LayerConv2D and LayerConvPool2D
    """
    def __init__(self, neuron_type, in_shape, flt_shape, strides, pads, save_output=False, in_block=None):
        """
        Args:
            in_shape (tuple or list of length 3):
            (num input feature maps, image height, image width)
    ...
    def forward(self, x, batch_size, run_time):
        ...
        # Convolve input feature maps with filters
        conv_out = conv2d(input=x,
                               filters=self.w,
                               input_shape=img_batch_shape,
                               filter_shape=self.filter_shape,
...
class LayerBlockPadConv2D(LayerBlockPadConv2DAbstract):
    """
    2D convolutional layer block without activation function with padding
    """
    name = "2D convolutional layer block without activation function with padding "

    def __init__(self, neuron_type, in_shape, flt_shape, strides, pads, save_output=False, in_block=None):
        LayerBlockPadConv2DAbstract.__init__(self, neuron_type, in_shape, flt_shape, strides, pads, save_output, in_block)
        output_width = int((in_shape[1] - flt_shape[2] + 2*pads[0])/strides[0] + 1)
        output_height = int((in_shape[2] - flt_shape[3] + 2*pads[1])/strides[1] + 1)
        self.output_shape = (flt_shape[0], output_width, output_height)

    def compute_bound_parameters_virtual(self):
        fan_in = np.prod(self.filter_shape[1:])
        fan_out = self.filter_shape[0] * np.prod(self.filter_shape[2:])

        # # if no padding, then the output size is halved
        # if not self.pads == (1,1):
        #     fan_out = fan_out/2
    ...
    def forward_virtual(self, conv_out):
        ...
        if self.save_output:
            if self.in_block:
                if hasattr(self.in_block, 'output'):
                    self.in_block.output = None
...
class LayerBlockNonLinear(LayerBlock):
    """
    Non-linear function layer
    """
    name = "Non-linear activation function layer"

    def __init__(self, neuron_type, save_output=False, in_block=None, filter_shape=None, output_shape=None):
        LayerBlock.__init__(self)
        self.neuron_type = neuron_type
        self.save_output = save_output
        self.in_block = in_block
    ...
    def forward(self, x, batch_size, run_time):
        if self.save_output:
            if self.in_block:
                if hasattr(self.in_block, 'output'):
                    self.in_block.output = None
            self.output = self.neuron_type.activation_function(x).flatten(2)
            return self.output
        else:
            return self.neuron_type.activation_function(x).flatten(2)
...
class LayerBlockConv2D(LayerBlockConv2DAbstract):
    ...
    def __init__(self, neuron_type, in_shape, flt_shape, save_output=False, in_block=None):
        LayerBlockConv2DAbstract.__init__(self, neuron_type, in_shape, flt_shape, save_output, in_block)
        output_width = int(in_shape[1] - flt_shape[2] + 1)
        output_height = int(in_shape[2] - flt_shape[3] + 1)
        self.output_shape = (flt_shape[0], output_width, output_height)
    ...
    def forward_virtual(self, conv_out):
        if self.save_output:
            if self.in_block:
                if hasattr(self.in_block, 'output'):
                    self.in_block.output = None
            self.output = self.neuron_type.activation_function(conv_out + self.b.dimshuffle('x', 0, 'x', 'x')).flatten(2)
            return self.output
        else:
            return self.neuron_type.activation_function(conv_out + self.b.dimshuffle('x', 0, 'x', 'x')).flatten(2)
...
class LayerBlockConvPool2D(LayerBlockConv2DAbstract):
    ...
    def __init__(self, neuron_type, in_shape, flt_shape, poolsize=(2, 2), save_output=False, in_block=None):
        self.poolsize = poolsize
        LayerBlockConv2DAbstract.__init__(self, neuron_type, in_shape, flt_shape, save_output, in_block)
        output_width = int((in_shape[1] - flt_shape[2] + 1)/poolsize[0])
        output_height = int((in_shape[2] - flt_shape[3] + 1)/poolsize[1])
        self.output_shape = (flt_shape[0], output_width, output_height)
    ...
    def forward_virtual(self, conv_out):
        ...
        if self.save_output:
            if self.in_block:
                if hasattr(self.in_block, 'output'):
                    self.in_block.output = None
            self.output = self.neuron_type.activation_function(pooled_out + self.b.dimshuffle('x', 0, 'x', 'x')).flatten(2)
            return self.outpu
        else:
